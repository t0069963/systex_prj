...
SELECT TransDate,CropName,MarketName,AVG(Trans_Quantity),max(Trans_Quantity),min(Trans_Quantity) FROM `agriproductstranstype` where MarketName = "台北二" and CropName = "椰子" and TransDate >='2020-01-02' and TransDate <='2021-10-04' ;
...
SELECT Trans_Quantity,TransDate  from `agriproductstranstype` where  TransDate >= date_sub('2021-07-07',INTERVAL 5 DAY) and TransDate <= '2021-07-07'and MarketName ="台北二" and CropName = "椰子" ORDER BY TransDate ASC;
